...
async fn verify(req: &mut Request, dep: &mut Depot) -> String {
    let verify_req = req.parse_body::<VerifyReq>().await.expect("Could not parse VerifyReq");
    let db = dep.obtain::<DB>().expect("get DB");
    let logs = db.select_logs().expect("get logs");
    let hashes = logs
        .iter()
    ...
    let verified = merkle::verify(&hashes, &verify_req.indices, &verify_req.hashes).expect("Could not verify");
...
pub async fn start_server(config: Config, db: DB) -> Result<()> {
    ...
    let router = Router::with_path("/api/v1")
        ...
        .push(Router::with_path("root").hoop(affix::inject(db.clone())).get(root))
        .push(Router::with_path("verify").hoop(affix::inject(db)).post(verify));
